...
<x-portal-layout :showHero="false">
    <div class="max-w-9xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-200">
            <form method="POST" action="{{ route('daily-reports.storeOrUpdate', ['user' => $user, 'date' => $dailyReport->report_date->format('Y-m-d')]) }}" class="flex flex-col h-full">
                @csrf
                ...
                <div class="p-8 md:p-12 flex-grow">
                    ...
                    @if ($yesterdayPlan)
                        <div class="mb-12 bg-yellow-50 border-l-8 border-yellow-400 p-8 rounded-r-2xl shadow-sm">
                            ...
                            <div class="text-lg leading-relaxed text-gray-800 bg-white p-6 rounded-xl border border-yellow-200 font-medium">
                    ...
                    <div class="grid grid-cols-1 xl:grid-cols-2 gap-x-12 gap-y-12">
                        <x-textarea-card
                            label="✅ 今日やったこと"
                            name="summary_today"
                            :value="$dailyReport->summary_today"
                        />
                        <x-textarea-card
                            label="📊 目標との差異"
                            name="discrepancy"
                            :value="$dailyReport->discrepancy"
                        />
                        <x-textarea-card
                            label="📝 明日やること"
                            name="summary_tomorrow"
                            :value="$dailyReport->summary_tomorrow"
                        />
                        <x-textarea-card
                            label="💭 困っていることや感想"
                            name="issues_thoughts"
                            :value="$dailyReport->issues_thoughts"
                        />
                    </div>
                </div>
